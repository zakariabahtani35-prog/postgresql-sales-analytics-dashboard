...
if not all([DB_USER, DB_PASSWORD, DB_HOST, DB_PORT, DB_NAME]):
    st.error("Missing database configuration in .env file.")
    st.info("Please create a .env file with DB_USER, DB_PASSWORD, DB_HOST, DB_PORT and DB_NAME.")
    st.stop()
...
try:
    ...
    st.info("Check your .env file and make sure PostgreSQL is running.")
    st.stop()
...
with col_a:
    sales_cat = (
        filtered_df.groupby("category_name")["sales"]
        .sum()
        .reset_index()
...
with col_b:
    profit_cat = (
        filtered_df.groupby("category_name")["profit"]
        .sum()
        .reset_index()
